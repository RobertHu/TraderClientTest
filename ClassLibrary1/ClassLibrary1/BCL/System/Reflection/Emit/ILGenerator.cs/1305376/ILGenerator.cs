...
{ 
    ...
    public class ILGenerator : _ILGenerator 
    {
        ...
        {
            Contract.Requires(!(method is DynamicMethod)); 
            ...
            { 
                ...
                if (!m_methodBuilder.Module.Equals(methodInfoUnbound.Module) 
                ...
                byte[] sigBytes = SignatureHelper.GetMethodSpecSigHelper(
                    modBuilder, methodInfo.GetGenericArguments()).InternalGetSignature(out sigLength); 

                // Create MethodSepc M<Bar> with parent G?.M<S> 
                tk = TypeBuilder.DefineMethodSpec(modBuilder.GetNativeHandle(), tk, sigBytes, sigLength); 
            }
            else 
            {
                ...
                { 
                    if (methodInfo != null)
                        tk = modBuilder.GetMethodTokenInternal(methodInfo).Token; 
                    else 
