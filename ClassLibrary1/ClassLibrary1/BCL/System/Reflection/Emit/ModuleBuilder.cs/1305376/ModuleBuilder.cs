...
[System.Runtime.InteropServices.ComVisible(true)]
    public class ModuleBuilder : Module, _ModuleBuilder 
    {
        ...
        { 
            m_TypeBuilderList.Add(type);
        } 
 
        internal void CheckTypeNameConflict(String strTypeName, TypeBuilder enclosingType)
        { 
            for (int j = 0; j < m_TypeBuilderList.Count; j++)
            {
                Type curType = (Type)m_TypeBuilderList[j];
                if (curType.FullName.Equals(strTypeName) && object.ReferenceEquals(curType.DeclaringType, enclosingType)) 
                {
        ...
        { 
            return GetMemberRefOfFieldInfo(GetNativeHandle(), tkType, declaringType, tkField);
        } 
        ...
        internal void Init(String strModuleName, String strFileName, int tkFile)
        {
            ...
            m_TypeBuilderList = new List<Type>();
        } 
        ...
        {
            int size = m_TypeBuilderList.Count; 
            List<Type> typeList = new List<Type>(size); 
        ...
        private FieldToken GetFieldTokenNoLock(FieldInfo field)
        {
            ...
            else if ( (rtField = field as RuntimeFieldInfo) != null) 
            {
                ...
                { 
                    ...
                    byte[] sig = SignatureHelper.GetTypeSigToken(this, field.DeclaringType).InternalGetSignature(out length); 
                    tr = GetTokenFromTypeSpec(sig, length);
                    mr = GetMemberRefOfFieldInfo(tr, field.DeclaringType.GetTypeHandleInternal(), fieldToken);
                }
                else 
                {
                    tr = GetTypeTokenInternal(field.DeclaringType).Token; 
