...
{ 
    public sealed class MemoryFailPoint : CriticalFinalizerObject, IDisposable 
    {
        ...
        public MemoryFailPoint(int sizeInMegabytes) 
        {
            ...
            ulong segmentSize = (ulong) (Math.Ceiling((float)size / GCSegmentSize) * GCSegmentSize); 
            ...
            for(int stage = 0; stage < 3; stage++) { 
                ...
                bool needPageFile = availPageFile < segPlusReserved + LowMemoryFudgeFactor || overflow; 
                ...
                BCLDebug.Trace("MEMORYFAILPOINT", "MemoryGate: Checking for {0} MB, for allocation size of {1} MB, stage {9}.  Need page file? {2}  Need Address Space? {3}  Need Contiguous address space? {4}  Avail page file: {5} MB  Total free VA space: {6} MB  Contiguous free address space (found): {7} MB  Reserved space: {8} MB", 
            ...
            Interlocked.Add(ref LastKnownFreeAddressSpace, -((long) size)); 
        ...
        { 
            ...
            IntPtr sizeOfMemInfo = (IntPtr) Marshal.SizeOf(memInfo);
 
            while (((ulong)address) + size < TopOfMemory) {
                IntPtr r = Win32Native.VirtualQuery(address, ref memInfo, sizeOfMemInfo); 
                if (r == IntPtr.Zero) 
                    __Error.WinIOError();
