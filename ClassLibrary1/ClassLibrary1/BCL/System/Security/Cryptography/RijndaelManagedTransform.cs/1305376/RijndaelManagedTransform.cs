                        ...
                        fixed (int *TF = s_TF) { 
                            ...
                            for (int blockNum = 0;  blockNum < iNumBlocks; ++blockNum) { 
                                ...
                                if (m_cipherMode == CipherMode.CFB) { 
                                    ...
                                    Buffer.memcpy(m_shiftRegister, 0, (byte *)work, 0, m_blockSizeBytes);
                                } else { 
#endif // FEATURE_CRYPTO
                                    if (blockNum != iNumBlocks - 1 || padSize == 0) {
                                        Contract.Assert(m_blockSizeBytes <= m_Nb * sizeof(int), "m_blockSizeBytes <= m_Nb * sizeof(int)");
                                        Buffer.memcpy(inputBuffer, workBaseIndex, (byte *)work, 0, m_blockSizeBytes); 
                                    } else {
                                        int padIndex = 0; 
                                        index = workBaseIndex; 
                                        for (int i = 0; i < m_Nb; ++i) {
                                ...
                                if (m_cipherMode == CipherMode.CFB) { 
                                    ...
                                    if (m_cipherMode == CipherMode.CBC) {
                                        ...
                                        fixed (int* pLastBlockBuffer = m_lastBlockBuffer) {
                                            ...
                                            Buffer.memcpyimpl((byte *)temp, (byte *)pLastBlockBuffer, m_blockSizeBytes);
                                        } 
#if FEATURE_CRYPTO
                                    } 
                                } 
