...
namespace TrainTicketLogin
{
    ...
    {
        private volatile bool _isLoginThreadStoped = true;
        public Form1()
        {
            InitializeComponent();
        }
        ...
        {
            if (!this._isLoginThreadStoped) return;
            // Verify login field
        ...
        private void Login()
        {
            this._isLoginThreadStoped = false;
            SetLoginResult("正在登陆中");

            string userName = ConfigurationManager.AppSettings["loginID"];
            ...
            for (int i = 0; i < 10000; i++)
            {
                ...
                {
                    // Get random number
                    if (this._isLoginThreadStoped) break;
                    ...
                    var result = Parser.IsLoginSuccess(afterLogin);
                    if (!result.Item1)
                    {
                        string msg = string.Format("登录失败，进行第{0}次重试,错误原因:{1}", i, result.Item2);
                        SetLoginResult(msg);
                    }
                    else
                    {
                        SetLoginResult("登陆成功 " + result.Item2);
                        break;
                    }
            ...
            this._isLoginThreadStoped = true;
        }
        ...
        private void btnOpenPage_Click(object sender, EventArgs e)
        {
            ...
            Process.Start("IExplore.exe", "http://dynamic.12306.cn/otsweb/");
        }
